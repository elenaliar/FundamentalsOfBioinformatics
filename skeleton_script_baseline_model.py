...
def parse_blosum(path):
    ...
    for i in range(len(aas)):
        ...
        score_row = aa_scores[i]
        for j in range(len(aas)):
            new_dict = {aas[j] : score_row[j]}
            blosum_dict[aas[i]].update(new_dict)
...
if __name__ == "__main__":
    #data/vep/HGVS_2020_small_VEP_baseline.tsv","data/BLOSUM62.txt","output/output.tsv"
    main()
